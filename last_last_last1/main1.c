...
int main(void) {
    ...
    SDL_Surface *border_s = IMG_Load("back_back_lvl_1.png");
    ...
    SDL_Texture *platform4 = SDL_CreateTextureFromSurface(renderer, area4_s); // texture for 4 plato in 4 block
    SDL_Texture *platform5 = SDL_CreateTextureFromSurface(renderer, area3_s);
    SDL_Texture *platform6 = SDL_CreateTextureFromSurface(renderer, bs_r); // texture for 5 plato
    SDL_Texture *border = SDL_CreateTextureFromSurface(renderer, border_s);
    SDL_FreeSurface(bs);
    ...
    Player player = {100, 500, 0, 0, spriteWidth * 2, spriteHeight * 2, spriteTexture};
    ...
    while (running) {
        ...
        if (player.x < 16 && player.y < SCREEN_HEIGHT) { //
            player.x = 16;
        }
        if (player.x + player.w > SCREEN_WIDTH - 16 && (player.y > 0 && player.y < 545)) {
            player.x = SCREEN_WIDTH - player.w - 16; //// next level and door
        }
        if (player.y < 16) { 
            player.y = 16;
        }

        if (player.y >  SCREEN_HEIGHT + 50 && player.x < SCREEN_WIDTH) { /// check x
            SDL_Delay(500);
            player.x = 100;
            player.y = 500;
        }
        ...
        /*if (player.y + player.h > SCREEN_HEIGHT) { //// reset
        ...
        SDL_Rect pRect_platform3 = {682, 450, 128, 16}; // plato 4 for 4
        SDL_Rect pRect_platform4 = {860, 556, 96, 16}; // plato 5
        ...
        SDL_RenderCopy(renderer, platform4, NULL, &pRect_platform3);
        SDL_RenderCopy(renderer, platform5, NULL, &pRect_platform4);
        SDL_RenderCopy(renderer, platform6, NULL, &pRect1_r);
        SDL_RenderCopy(renderer, border, NULL, &Rect_bord); //////
        /*for (unsigned long i = 0; i < sizeof(platforms) / sizeof(Platform); i++) {
