...
void render_arena_1(SDL_Renderer * renderer, Platform **platforms, SDL_Texture *platforms_t[])
{
    /**platforms = (Platform*)malloc(sizeof(Platform)*4);
    (*platforms)[0] = (Platform){0, 656, 288, 64, {255, 255, 255, 255}};
    (*platforms)[1] = (Platform){300, 556, 96, 16, {255,255,255,255}};
    (*platforms)[2] = (Platform){500, 556, 96, 16, {255,255,255,255}};
    (*platforms)[3] = (Platform){1280-288, 656, 288, 64, {255,255,255,255}};*/
    ...
    SDL_RenderCopy(renderer, platforms_t[3], NULL, &pRect1_r);
}

void render_arena_2(SDL_Renderer * renderer, Platform **platforms, SDL_Texture *platforms_t[])
{
    *platforms = (Platform*)malloc(sizeof(Platform)*1);
    /*
    SDL_Rect pRect1 = {0, 656, 288, 64};
    SDL_Rect pRect1_l = {0, 656, 288, 64};
    SDL_Rect pRect1_r = {1280-288, 656, 288, 64};
...
void fillPlatforms(Platform **platforms, int size) {
    // Освобождаем память, если массив уже был создан ранее
    if (*platforms != NULL) {
        free(*platforms);
    }
    
    // Выделяем память под новый массив
    *platforms = (Platform*)malloc(size * sizeof(Platform));
    
    // Проверяем, удалось ли выделить память
    if (*platforms == NULL) {
        // Обработка ошибки, если выделение памяти не удалось
        printf("Ошибка: не удалось выделить память для массива Platform\n");
...
int main(void) {
    ...
    Player player = {320, 240, 0, 0, 50, 100, spriteTexture};
    ...
    while (running) {
        ...
        if (arena_switch == 1) {
            ...
            fillPlatforms(&platforms,4);
            for (int i = 0; i < 4; i++) {
                (*platforms)[i] = (Platform*){i * 100, 656, 288, 64, {255, 255, 255, 255}};
            }
        ...
        else
        {
            ...
            render_arena_2(renderer, &platforms, platforms_t);
            fillPlatforms(&platforms,1);
            for (int i = 0; i < 1; i++) {
                (*platforms)[i] = (Platform*){i * 100, 656, 288, 64, {255, 255, 255, 255}};
            }
